...
class SpatioTemporalBlock(nn.Module):
    ...
    def forward(self, x: Tensor, causal_temporal: bool = False) -> Tensor:
        ...
        x_ = self.norm1(x)
        x_ = self.spatial_attn(x_, is_causal=False)
        x = x + x_
...
class SpatioTemporalTransformer(nn.Module):
    ...
    def forward(self, x: Tensor, lang_embed: Tensor = None) -> Tensor:
        x = self.ffn(x)
        x = self.pos_enc(x)
...
class MVSpatioTemporalTransformer(nn.Module):
    ...
    def forward(self, latent_action: Tensor, view1: Tensor, view2: Tensor, lang_embed: Tensor = None) -> Tensor:
...
class MVSpatioTransformer(nn.Module):
    ...
    def forward(self, latent_action: Tensor, view1: Tensor, lang_embed: Tensor = None) -> Tensor:
        view1 = self.ffn(view1) #+ repeat(self.view_embed[0], 'd -> b m n d', b = view1.shape[0], m = view1.shape[1], n=1)
        # view2 = self.ffn(view2) + repeat(self.view_embed[1], 'd -> b m n d', b = view1.shape[0], m = view1.shape[1], n=1)
