...
def resolve_vl_layer_selection(cfg, *, num_vl_layers: int):
    """
    Resolve ``framework.action_model.{num_dit_layers, vl_layer_indices}`` into a
    concrete ``(num_dit_layers, vl_layer_indices)`` pair.

    Both keys are optional and default to the legacy QwenPI behaviour:
        num_dit_layers   = num_vl_layers
        vl_layer_indices = [0, 1, ..., num_vl_layers - 1]   (every VLM block)

    Semantics:
        - ``vl_layer_indices`` are 0-indexed over VLM transformer blocks
          (0 = first block output, ``num_vl_layers - 1`` = last).  Negative
          indices are supported (-1 = last block).
        - When only ``num_dit_layers`` is set, the last K VLM blocks are used
          in order — matching the original ``hidden_states[-K:]`` slicing.
        - When both are set their lengths must agree.
        - Out-of-range or zero/negative depth raise ``ValueError``.

    The resolved values are written back into the config so they end up in
    saved ``config.yaml`` snapshots.

    Returns:
        Tuple[int, List[int]]: (num_dit_layers, vl_layer_indices).
    """
    action_cfg = cfg.framework.action_model
    num_dit_layers_cfg = action_cfg.get("num_dit_layers", None)
    vl_layer_indices_cfg = action_cfg.get("vl_layer_indices", None)

    if vl_layer_indices_cfg is not None:
        vl_layer_indices = [int(i) for i in vl_layer_indices_cfg]
        vl_layer_indices = [i if i >= 0 else num_vl_layers + i for i in vl_layer_indices]
        for i in vl_layer_indices:
            if not 0 <= i < num_vl_layers:
                raise ValueError(
                    f"vl_layer_indices entry {i} out of range for num_vl_layers={num_vl_layers}"
                )
        num_dit_layers = (
            int(num_dit_layers_cfg) if num_dit_layers_cfg is not None else len(vl_layer_indices)
        )
        if num_dit_layers != len(vl_layer_indices):
            raise ValueError(
                f"num_dit_layers ({num_dit_layers}) must equal len(vl_layer_indices) "
                f"({len(vl_layer_indices)}) when both are set."
            )
    else:
        num_dit_layers = int(num_dit_layers_cfg) if num_dit_layers_cfg is not None else num_vl_layers
        if not 1 <= num_dit_layers <= num_vl_layers:
            raise ValueError(
                f"num_dit_layers={num_dit_layers} must be in [1, num_vl_layers={num_vl_layers}] "
                "when vl_layer_indices is not specified."
            )
        vl_layer_indices = list(range(num_vl_layers - num_dit_layers, num_vl_layers))

    cfg.framework.action_model.num_dit_layers = num_dit_layers
    cfg.framework.action_model.vl_layer_indices = list(vl_layer_indices)
    return num_dit_layers, vl_layer_indices
