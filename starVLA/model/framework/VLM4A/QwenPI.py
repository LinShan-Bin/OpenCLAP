...
class QwenPIDefaultConfig:
    ...
    action_model: dict = field(
        default_factory=lambda: {
            ...
            # Number of DiT cross-attention layers.  None => use the VLM's
            # num_hidden_layers (preserves the original QwenPI behaviour).
            "num_dit_layers": None,
            # Which VLM transformer-block outputs feed each DiT layer, in order.
            # 0-indexed: 0 = first VLM block output, num_vl_layers-1 = last.
            # Negative indices are supported (-1 = last block).
            # None => use the last `num_dit_layers` blocks (original behaviour).
            # If both num_dit_layers and vl_layer_indices are set, their lengths
            # must agree.
            "vl_layer_indices": None,
...
class Qwen_PI(baseframework):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Resolve DiT depth and which VLM layers feed it.
        # Defaults preserve the original behaviour: one DiT layer per VLM layer,
        # consuming the last `num_vl_layers` block outputs in order.
    ...
    def _encode_vl_hidden_states(
        ...
        with torch.autocast("cuda", dtype=torch.bfloat16):
            ...
            # embedding output, index k+1 is the output of VLM block k).  Our
            # `vl_layer_indices` are 0-indexed over VLM blocks, so we offset by 1.
